												...
												<div class="form-step">
													...
													<template v-if="conferma === true" v-cloak>
														<div class="alert fade show alert-success" role="alert">
															<p>
																...
																Accettazione del posto per la domanda n&deg; <strong>{{preferenza.domanda.protocollo}}</strong> effettuata il <span v-html="$options.filters.dataOraRicevuta(preferenza.domanda.elencoPreferenze[preferenza.posto].dataUltimoCambioStato)"></span>
															</p>
														</div>
														...
														<p>Hai <strong>accettato</strong> il posto
														...
														<div class="detail">
															...
															<!-- p v-if="['M','S','A','P'].indexOf(preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola) !== -1">Sei invitato/a a contattare telefonicamente
																<template v-if="preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola == 'P'">
																...
																entro 3 giorni per fissare un appuntamento in cui ti saranno fornite le informazioni necessarie per iniziare la frequenza del bambino o della bambina.</p -->
															<p v-if="['M','S','A','P'].indexOf(preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola) !== -1">Sarai contattato dalla Divisione Servizi Educativi della Citt&agrave; di Torino per fissare un appuntamento in cui ti saranno fornite tutte le informazioni necessarie per iniziare la frequenza del bambino o della bambina.</p>
															...
															<p>L'adempimento degli obblighi vaccinali &egrave; indispensabile per frequentare il nido d'infanzia.</p>
														</div>
														<div class="info-accettazione">
															<h3>Informazioni utili</h3>
															<template v-if="['M','S','A'].indexOf(preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola) !== -1">
															...
															<template v-if="['M','S'].indexOf(preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola) !== -1">
															...
															<h4>Tariffa</h4>
															<p v-if="preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola == 'P'">
																La tariffa viene stabilita dal gestore in base a quanto definito nella Convenzione con la Citt&agrave;. Maggiori informazioni sul sito della Citt&agrave;, nella sezione dedicata ai Servizi Educativi 0-2 anni.
															</p>
															<p v-else>Con l'iscrizione al servizio viene assegnata la tariffa completa. I residenti a Torino possono fruire delle tariffe agevolate.<br />
															...
															<p v-if="preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola == 'C'">Devi presentare la copia della richiesta e dell'attestazione ISEE al nido d'infanzia convenzionato.</p>
															<!-- template v-if="['M','S','A'].indexOf(preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola) !== -1">
																...
																Possono accedere al <strong>Buono Servizi Prima Infanzia</strong> bambini e bambine residenti in Piemonte che frequentano un nido comunale.</p>
															</template -->
														</div>
													...
													<template v-else v-cloak>
														...
														<p>Stai <strong>accettando</strong> il posto
															<template v-if="preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola == 'P'">
																nella sezione primavera
															</template>
															<template v-else>
														...
														<div class="detail">
															<h3>{{ preferenza.domanda.elencoPreferenze[preferenza.posto].descrizione }} <span class="badge badge-info" v-if="['C','P'].indexOf(preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola) !== -1">{{ preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola | categoriaScuola }}</span></h3>
															<p>
																<strong>Indirizzo</strong>: {{ preferenza.domanda.elencoPreferenze[preferenza.posto].indirizzo }}<br />
																<strong>Tempo frequenza</strong>: {{ preferenza.domanda.elencoPreferenze[preferenza.posto].codTipoFrequenza | tipoFrequenza }}<br />
															</p>
														...
														<div class="form-row mt-8">
															<div class="form-group col-sm-12 col-md-6">
															  <label for="n-telefono" class="col-form-label">Numero di telefono<sub class="required">*</sub></label>
																...
																<small id="telefonoHelp" class="form-text text-muted">il numero inserito &egrave; quello sul quale accetto di ricevere comunicazioni nido/famiglia</small>
															</div>
														</div>
														<template v-if="['M','S','A'].indexOf(preferenza.domanda.elencoPreferenze[preferenza.posto].codCategoriaScuola) !== -1">
