<div class="form-step">
	<h2 class="no-bottom">Figli o figlie frequentanti o in corso di iscrizione</h2>
	<div class="alert alert-warning fade show" role="alert">
		...
		<ul>
			<li>eventuali figli gi&agrave; frequentanti la scuola selezionata come prima scelta che continueranno la frequenza nell'anno scolastico per il quale viene presentata la domanda;</li>
			<li>eventuali altri figli per i quali si presenta la domanda di iscrizione per la stessa scuola.</li>
		</ul>
	</div>
	<div class="form-row">
	...
	<template v-if="(elencoFigliFrequentantiContiguo.length > 0 && domanda.elencoMaterne[0].contiguo.descrizione !== '')">
		<h2 class="mt-8">Figli o figlie frequentanti un nido comunale contiguo</h2>
		<div class="form-row">		
			<div id="fratello-contiguo" class="form-group col-md-12">
				<label class="col-form-label label-light col-md-12">Esistono altri figli o figlie frequentanti il nido di <strong>{{ domanda.elencoMaterne[0].contiguo.indirizzo }}</strong>?<sub class="required">*</sub></label>
				<div class="custom-control custom-radio custom-control-inline">
					<input type="radio" id="fratello-contiguo-si" name="fratello-contiguo" :class="{'custom-control-input':true, 'is-invalid': errors.has('fratello-contiguo')}" :value="true" v-model="domanda.minore.fratelloNidoContiguo.stato" v-validate.disable="'required'" />
					<label class="custom-control-label" for="fratello-contiguo-si">Si</label>
				</div>
				<div class="custom-control custom-radio custom-control-inline">
					<input type="radio" id="fratello-contiguo-no" name="fratello-contiguo" :class="{'custom-control-input':true, 'is-invalid': errors.has('fratello-contiguo')}" :value="false" v-model="domanda.minore.fratelloNidoContiguo.stato" />
					<label class="custom-control-label" for="fratello-contiguo-no">No</label>
				</div>
				<span class="invalid-feedback" v-show="errors.has('fratello-contigo')">{{ errors.first('fratello-contiguo') }}</span>
			</div>
			<template v-if="domanda.minore.fratelloNidoContiguo.stato === true">
				<div id="figlio-contiguo" class="form-group col-md-12">
					<label class="col-form-label label-light col-md-12">Scegli un figlio o una figlia tra quelli dichiarati in precedenza<sub class="required">*</sub></label>
					<hr />
					<div v-for="(figlio, index) in elencoFigliFrequentantiContiguo" :key="index" class="custom-control custom-radio">
						<input type="radio" :id="'figlio-contiguo'+index" name="figlio-contiguo" :class="{'custom-control-input':true, 'is-invalid': errors.has('figlio-contiguo')}" :value="figlio.codiceFiscale" v-model="domanda.minore.fratelloNidoContiguo.anagrafica.codiceFiscale" v-validate.disable="'required'" @change="setFigliFrequentantiContiguo" />
						<label class="custom-control-label" :for="'figlio-contiguo'+index">
							<big><strong>{{ figlio.cognome }} {{ figlio.nome }}</strong></big><br />
							{{ figlio.codiceFiscale }}<br />
							<template v-if="figlio.sesso == 'M'">
								nato
					...
					<span v-show="errors.has('figlio-contiguo')" class="invalid-feedback ml-4">{{ errors.first('figlio-contiguo') }}</span>				
				</div>
			</template>
		</div>
	</template>
